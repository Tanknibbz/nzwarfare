...
if (isNil {missionNamespace getVariable "WFBE_C_AI_MAX"}) then {missionNamespace setVariable ["WFBE_C_AI_MAX", 16]}; //--- Max AI allowed on each AI groups.
...
switch (true) do {
	case (WF_A2_Vanilla): {
		...
		missionNamespace setVariable ["WFBE_C_UNITS_FACTIONS_WEST", ['CDF','ANZAC']]; //--- West Factions.
	};
	case (WF_A2_Arrowhead): {
		...
		missionNamespace setVariable ["WFBE_C_UNITS_FACTIONS_WEST", ['ANZAC']]; //--- West Factions.
	};
